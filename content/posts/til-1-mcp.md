...
Eg: If i need to read messages from slack , I need to write a readMessage tool using the /GET API from slack  and so on for other tools 
- Now this means that every time I create my application I need to include this code as well , and if the Provider(Slack) changes their API , then I as well need to change my code 
- Also everyone writes their own code in their app so pretty inconsistent since everyone has their own way of writing the tool : Eg the schema of output returned by A code is different than B code even though both use same API for same tool


## After MCP

MCP just standardizes this whole process, as now someone (eg provider eg: Slack) create their MCP server and put all the low level implementation inside it and exposes the standard interface/schema to users 
Now everyone can easily connect to it and if they change their API later , you do not have to change your own code cause  whoever wrote it, they will change their low level implementation while the interface remains standard
...
Eg: When connected to Airline mcp server , LLM application can not just retrieve information but also perform task like book a flight etc. 
...
Ex: JSON-RPC call from an client to call tool on a mcp server:
```
{
  "jsonrpc": "2.0",
  "method": "tools/call",
...
5. Server receives the message from client over protocol , then runs the function and then return the output schema back to client
...
Eg: read_file tool accepts a filename and return the content of the file , similarly web_search tool etc 
...
_Note: How with each step AI is perceiving the info and deciding what it need to do in a loop until it achieves the goal_
